...
/**
 * GET /api/econt/quarters?city_id={id}
 * Proxy request to backend Econt quarters endpoint (server-side)
 */
export async function GET(request: NextRequest) {
  try {
    ...
    const response = await fetch(
      `${BACKEND_URL}/store/econt/quarters?city_id=${cityId}`,
      {
        method: "GET",
        headers: {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { message: errorText || "Failed to fetch quarters" },
