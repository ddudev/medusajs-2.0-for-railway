...
/**
 * GET /api/econt/streets?city_id={id}
 * Proxy request to backend Econt streets endpoint (server-side)
 */
export async function GET(request: NextRequest) {
  try {
    ...
    const response = await fetch(
      `${BACKEND_URL}/store/econt/streets?city_id=${cityId}`,
      {
        method: "GET",
        headers: {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { message: errorText || "Failed to fetch streets" },
    ...
    console.error("Error proxying Econt streets request:", error)
    return NextResponse.json(
